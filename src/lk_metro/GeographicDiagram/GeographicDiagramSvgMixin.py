...
class GeographicDiagramSvgMixin:
    def _svg_style_lines(self) -> list[str]:
        return [
            ...
            f".station {{ stroke-width: {self.STATION_TICK_STROKE_WIDTH}; stroke-linecap: square; }}",
            f".interchange {{ fill: white; stroke: #000000; stroke-width: {
                self.INTERCHANGE_STROKE_WIDTH}; }}",
            f".label {{ font: {
                self.LABEL_FONT_SIZE}px {
                self.FONT_FAMILY}; fill: {
                self.LABEL_COLOR}; dominant-baseline: middle; }}",
            f".map-title {{ font: bold {
                self.TITLE_FONT_SIZE}px {
                self.FONT_FAMILY}; fill: {
                self.TEXT_COLOR}; }}",
            f".legend-label {{ font: {
                self.LEGEND_FONT_SIZE}px {
                self.FONT_FAMILY}; fill: {
                self.TEXT_COLOR}; dominant-baseline: middle; }}",
            f".legend-route-label {{ font: {
                self.LEGEND_FONT_SIZE}px {
                self.FONT_FAMILY}; fill: {
                self.LABEL_COLOR}; dominant-baseline: middle; }}",
        ]
    ...
    def _svg_header_lines(self) -> list[str]:
        ...
        return [
            ...
            *self._svg_style_lines(),
            "</style>",
            f'<rect width="{svg_width}" height="{svg_height}" fill="{
    ...
    def _route_svg_lines(
        ...
        for route in self.routes:
            ...
            lines.append(
                f'<polyline class="route" points="{points}" stroke="{
                    route.color}" stroke-width="{
                    self.ROUTE_STROKE_WIDTH}"/>'
            )
        return lines
    ...
    def _grid_svg_lines(self) -> list[str]:
        ...
        for x in range(0, self.width + 1, GRID_SPACING):
            ...
            lines.append(
                f'<line class="{grid_class}" x1="{x}" y1="0" x2="{x}" y2="{
                    self.height}"/>'
            )
        for y in range(0, self.height + 1, GRID_SPACING):
            ...
            lines.append(
                f'<line class="{grid_class}" x1="0" y1="{y}" x2="{
                    self.width}" y2="{y}"/>'
            )
        lines.append("</g>")
        return lines
