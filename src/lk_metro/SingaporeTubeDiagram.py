...
class SingaporeTubeDiagram(HarryBeckDiagram):
    ...
    def _content_dimensions(self) -> tuple[int, int]:
        return (
            self.width,
            self.height + self.TITLE_HEIGHT + self.FOOTER_HEIGHT,
        )
    ...
    def _code_badge_svg_lines(
        ...
        total_width = sum(
            width for _, _, width in badges
        ) + self.CODE_BADGE_GAP * (len(badges) - 1)
        ...
        for code, color, width in badges:
            lines.extend(
                [
                    ...
                    f'<text class="station-code-text" x="{
                        start_x + width / 2}" '
    ...
    def _title_and_legend_svg_lines(self) -> list[str]:
        lines = [
            ...
            f'<text class="system-title" x="13" y="8.7">{
                self.MAP_TITLE}</text>',
            f'<rect class="footer" x="0" y="{
        ...
        for index, route in enumerate(self.legend_routes):
            ...
            lines.extend(
                [
                    ...
                    f'y="{
                        y_coordinate -
                        0.28}" text-anchor="middle">{line_code}</text>',
    ...
    def to_svg(self) -> str:
        ...
        for stop in self.stops:
            ...
            route_ids = sorted(
                memberships[stop.name], key=self._route_order.__getitem__
            )
            if len(route_ids) > 1:
