import heapq
import itertools
import json
import math
from collections import defaultdict
from dataclasses import dataclass
from pathlib import Path

import networkx as nx

ROOT_DIR = Path(__file__).resolve().parents[1]
DIRECTIONS = {
    (1, 0): "E",
    (1, 1): "SE",
    (0, 1): "S",
    (-1, 1): "SW",
    (-1, 0): "W",
    (-1, -1): "NW",
    (0, -1): "N",
    (1, -1): "NE",
}
PORT_LENGTH = 256


@dataclass
class _CorridorSearch:
    start: tuple[int, int]
    end: tuple[int, int]
    bounds: tuple[int, int, int, int]
    blocked: set[tuple[int, int]]
    segments: dict[tuple[tuple[int, int], tuple[int, int]], tuple[str, str]]
    queue: list[tuple[int, int, int, tuple[int, int]]]
    previous: dict[tuple[int, int], tuple[int, int] | None]
    distances: dict[tuple[int, int], int]


def edge_key(first: str, second: str) -> tuple[str, str]:
    return tuple(sorted((first, second)))


def digital_line(
    start: tuple[int, int],
    end: tuple[int, int],
) -> list[tuple[int, int]]:
    x_coordinate, y_coordinate = start
    x_delta = abs(end[0] - x_coordinate)
    y_delta = abs(end[1] - y_coordinate)
    x_step = 1 if x_coordinate < end[0] else -1
    y_step = 1 if y_coordinate < end[1] else -1
    error = x_delta - y_delta
    path = [start]
    while (x_coordinate, y_coordinate) != end:
        doubled_error = error * 2
        if doubled_error > -y_delta:
            error -= y_delta
            x_coordinate += x_step
        if doubled_error < x_delta:
            error += x_delta
            y_coordinate += y_step
        path.append((x_coordinate, y_coordinate))
    return path


def route_corridor(
    start: tuple[int, int],
    end: tuple[int, int],
    blocked: set[tuple[int, int]],
    segments: dict[tuple[tuple[int, int], tuple[int, int]], tuple[str, str]],
) -> list[tuple[int, int]]:
    padding = 32
    min_x, max_x = sorted((start[0], end[0]))
    min_y, max_y = sorted((start[1], end[1]))
    search = _CorridorSearch(
        start=start,
        end=end,
        bounds=(
            min_x - padding,
            max_x + padding,
            min_y - padding,
            max_y + padding,
        ),
        blocked=blocked,
        segments=segments,
        queue=[(0, 0, 0, start)],
        previous={start: None},
        distances={start: 0},
    )
    return _search_corridor(search)


def _search_corridor(search: _CorridorSearch) -> list[tuple[int, int]]:
    while search.queue:
        _, _, distance, current = heapq.heappop(search.queue)
        if current == search.end:
            return _reconstruct_path(search.previous, current)
        if distance != search.distances[current]:
            continue
        for delta in DIRECTIONS:
            _queue_corridor_candidate(search, current, delta, distance)
    raise RuntimeError(
        f"Could not route corridor from {search.start} to {search.end}"
    )


def _reconstruct_path(
    previous: dict[tuple[int, int], tuple[int, int] | None],
    current: tuple[int, int],
) -> list[tuple[int, int]]:
    path = []
    while current is not None:
        path.append(current)
        current = previous[current]
    return list(reversed(path))


def _queue_corridor_candidate(
    search: _CorridorSearch,
    current: tuple[int, int],
    delta: tuple[int, int],
    distance: int,
) -> None:
    candidate = (current[0] + delta[0], current[1] + delta[1])
    if not _candidate_is_available(search, current, candidate, delta):
        return
    candidate_distance = distance + 1
    if candidate_distance >= search.distances.get(candidate, math.inf):
        return
    search.distances[candidate] = candidate_distance
    search.previous[candidate] = current
    heuristic = max(
        abs(candidate[0] - search.end[0]),
        abs(candidate[1] - search.end[1]),
    )
    deviation = abs(
        (candidate[0] - search.start[0]) * (search.end[1] - search.start[1])
        - (candidate[1] - search.start[1]) * (search.end[0] - search.start[0])
    )
    heapq.heappush(
        search.queue,
        (
            candidate_distance + heuristic,
            deviation,
            candidate_distance,
            candidate,
        ),
    )


def _candidate_is_available(
    search: _CorridorSearch,
    current: tuple[int, int],
    candidate: tuple[int, int],
    delta: tuple[int, int],
) -> bool:
    min_x, max_x, min_y, max_y = search.bounds
    is_within_bounds = (
        min_x <= candidate[0] <= max_x and min_y <= candidate[1] <= max_y
    )
    is_unblocked = candidate == search.end or candidate not in search.blocked
    crosses_diagonal = _crosses_opposite_diagonal(
        current, candidate, delta, search.segments
    )
    return is_within_bounds and is_unblocked and not crosses_diagonal


def _crosses_opposite_diagonal(
    current: tuple[int, int],
    candidate: tuple[int, int],
    delta: tuple[int, int],
    segments: dict[tuple[tuple[int, int], tuple[int, int]], tuple[str, str]],
) -> bool:
    if not delta[0] or not delta[1]:
        return False
    opposite = tuple(
        sorted(
            (
                (current[0], candidate[1]),
                (candidate[0], current[1]),
            )
        )
    )
    return opposite in segments


def compress_tokens(tokens: list[str]) -> str:
    compressed = []
    index = 0
    while index < len(tokens):
        token = tokens[index]
        next_index = index + 1
        while next_index < len(tokens) and tokens[next_index] == token:
            next_index += 1
        count = next_index - index
        compressed.append(f"{count if count > 1 else ''}{token}")
        index = next_index
    return "-".join(compressed)


def assign_ports(
    graph: nx.Graph,
    positions: dict[str, tuple[int, int]],
) -> dict[tuple[str, str], tuple[int, int]]:
    ports = {}
    for name in graph:
        origin = positions[name]
        neighbors = sorted(
            graph[name],
            key=lambda neighbor: math.atan2(
                positions[neighbor][1] - origin[1],
                positions[neighbor][0] - origin[0],
            ),
        )
        target_angles = [
            math.atan2(
                positions[neighbor][1] - origin[1],
                positions[neighbor][0] - origin[0],
            )
            for neighbor in neighbors
        ]
        directions = min(
            itertools.permutations(DIRECTIONS, len(neighbors)),
            key=lambda candidates: sum(
                abs(
                    math.atan2(
                        math.sin(
                            math.atan2(candidate[1], candidate[0])
                            - target_angle
                        ),
                        math.cos(
                            math.atan2(candidate[1], candidate[0])
                            - target_angle
                        ),
                    )
                )
                for candidate, target_angle in zip(candidates, target_angles)
            ),
        )
        for neighbor, direction in zip(neighbors, directions):
            ports[(name, neighbor)] = direction
    return ports


def main() -> None:
    routes = json.loads((ROOT_DIR / "data" / "routes.json").read_text())
    edge_routes, graph = _route_graph(routes)
    positions = _planar_positions(graph)
    ports = assign_ports(graph, positions)
    port_lengths = _port_lengths(ports, positions)
    _trim_port_lengths(ports, port_lengths, positions)
    paths = _draw_edge_paths(edge_routes, positions, ports, port_lengths)
    design = {
        "origin_stops": {
            name: list(point) for name, point in positions.items()
        },
        "routes": _design_routes(routes, paths, positions),
        "routes_backup_ignore": {},
    }
    print(json.dumps(design, indent=2))


def _route_graph(
    routes: list[dict[str, object]],
) -> tuple[dict[tuple[str, str], list[str]], nx.Graph]:
    edge_routes = defaultdict(list)
    graph = nx.Graph()
    for route in routes:
        for first, second in zip(route["stops"], route["stops"][1:]):
            graph.add_edge(first, second)
            edge_routes[edge_key(first, second)].append(route["id"])
    return edge_routes, graph


def _planar_positions(graph: nx.Graph) -> dict[str, tuple[int, int]]:
    is_planar, embedding = nx.check_planarity(graph)
    if not is_planar:
        raise RuntimeError("Route graph is not planar")
    planar_positions = nx.combinatorial_embedding_to_pos(
        embedding,
        fully_triangulate=True,
    )
    positions = {
        name: tuple(value * 20 for value in point)
        for name, point in planar_positions.items()
    }
    if len(set(positions.values())) != len(positions):
        raise RuntimeError("Station positions overlap")
    return positions


def _port_lengths(
    ports: dict[tuple[str, str], tuple[int, int]],
    positions: dict[str, tuple[int, int]],
) -> dict[tuple[str, str], int]:
    return {
        (name, neighbor): min(
            PORT_LENGTH,
            max(
                1,
                max(
                    abs(positions[name][index] - positions[neighbor][index])
                    for index in range(2)
                )
                // 3,
            ),
        )
        for name, neighbor in ports
    }


def _trim_port_lengths(
    ports: dict[tuple[str, str], tuple[int, int]],
    port_lengths: dict[tuple[str, str], int],
    positions: dict[str, tuple[int, int]],
) -> None:
    station_by_position = {point: name for name, point in positions.items()}
    for (name, neighbor), direction in ports.items():
        for distance in range(1, port_lengths[(name, neighbor)] + 1):
            point = tuple(
                positions[name][index] + direction[index] * distance
                for index in range(2)
            )
            if (
                point in station_by_position
                and station_by_position[point] != name
            ):
                port_lengths[(name, neighbor)] = max(1, distance - 1)
                break


def _draw_edge_paths(
    edge_routes: dict[tuple[str, str], list[str]],
    positions: dict[str, tuple[int, int]],
    ports: dict[tuple[str, str], tuple[int, int]],
    port_lengths: dict[tuple[str, str], int],
) -> dict[tuple[str, str], list[tuple[int, int]]]:
    paths = {}
    occupied = {point: name for name, point in positions.items()}
    drawn_segments = {}
    for edge, route_ids in sorted(
        edge_routes.items(),
        key=lambda item: -len(item[1]),
    ):
        path = _edge_path(edge, positions, ports, port_lengths)
        _record_path(edge, path, occupied, drawn_segments)
        paths[edge] = path
    return paths


def _edge_path(
    edge: tuple[str, str],
    positions: dict[str, tuple[int, int]],
    ports: dict[tuple[str, str], tuple[int, int]],
    port_lengths: dict[tuple[str, str], int],
) -> list[tuple[int, int]]:
    first, second = edge
    first_ray = _port_ray(first, second, positions, ports, port_lengths)
    second_ray = _port_ray(second, first, positions, ports, port_lengths)
    middle = digital_line(first_ray[-1], second_ray[-1])
    return [
        positions[first],
        *first_ray,
        *middle[1:-1],
        *reversed(second_ray),
        positions[second],
    ]


def _port_ray(
    name: str,
    neighbor: str,
    positions: dict[str, tuple[int, int]],
    ports: dict[tuple[str, str], tuple[int, int]],
    port_lengths: dict[tuple[str, str], int],
) -> list[tuple[int, int]]:
    return [
        tuple(
            positions[name][index] + ports[(name, neighbor)][index] * distance
            for index in range(2)
        )
        for distance in range(1, port_lengths[(name, neighbor)] + 1)
    ]


def _record_path(
    edge: tuple[str, str],
    path: list[tuple[int, int]],
    occupied: dict[tuple[int, int], object],
    drawn_segments: dict[
        tuple[tuple[int, int], tuple[int, int]], tuple[str, str]
    ],
) -> None:
    for point in path[1:-1]:
        if point in occupied:
            raise RuntimeError(
                f"Edge {edge} overlaps {occupied[point]!r} at {point}"
            )
        occupied[point] = edge
    for start, end in zip(path, path[1:]):
        segment = tuple(sorted((start, end)))
        if segment in drawn_segments:
            raise RuntimeError(
                f"Edges {edge} and {drawn_segments[segment]} overlap"
            )
        drawn_segments[segment] = edge


def _design_routes(
    routes: list[dict[str, object]],
    paths: dict[tuple[str, str], list[tuple[int, int]]],
    positions: dict[str, tuple[int, int]],
) -> dict[str, str]:
    design_routes = {}
    for route in routes:
        tokens = _route_tokens(route["stops"], paths, positions)
        design_routes[route["id"]] = compress_tokens(tokens)
    return design_routes


def _route_tokens(
    stops: list[str],
    paths: dict[tuple[str, str], list[tuple[int, int]]],
    positions: dict[str, tuple[int, int]],
) -> list[str]:
    tokens = []
    for first, second in zip(stops, stops[1:]):
        path = paths[edge_key(first, second)]
        if path[0] != positions[first]:
            path = list(reversed(path))
        for index, (start, end) in enumerate(zip(path, path[1:])):
            delta = (end[0] - start[0], end[1] - start[1])
            prefix = "b" if index < len(path) - 2 else ""
            tokens.append(prefix + DIRECTIONS[delta])
    return tokens


if __name__ == "__main__":
    main()
