...
def route_corridor(
	...
	while queue:
		...
		for delta in DIRECTIONS:
			...
			heuristic = max(abs(candidate[0] - end[0]), abs(candidate[1] - end[1]))
			deviation = abs(
	...
	for name in graph:
		...
		directions = min(
			...
			key=lambda candidates: sum(
				abs(
					math.atan2(
						math.sin(math.atan2(candidate[1], candidate[0]) - target_angle),
						math.cos(math.atan2(candidate[1], candidate[0]) - target_angle),
					)
				)
...
def main() -> None:
	...
	for (name, neighbor), direction in ports.items():
		for distance in range(1, port_lengths[(name, neighbor)] + 1):
			...
			if point in station_by_position and station_by_position[point] != name:
	...
	for edge, route_ids in sorted(
		...
		for point in path[1:-1]:
			if point in occupied:
				raise RuntimeError(f"Edge {edge} overlaps {occupied[point]!r} at {point}")
			occupied[point] = edge
		for start, end in zip(path, path[1:]):
			segment = tuple(sorted((start, end)))
			if segment in drawn_segments:
				raise RuntimeError(f"Edges {edge} and {drawn_segments[segment]} overlap")
	...
	for route in routes:
		...
		for first, second in zip(route["stops"], route["stops"][1:]):
			...
			for index, (start, end) in enumerate(zip(path, path[1:])):
				...
				tokens.append(("b" if index < len(path) - 2 else "") + DIRECTIONS[delta])
		design_routes[route["id"]] = compress_tokens(tokens)

	design = {
		"origin_stops": {name: list(point) for name, point in positions.items()},
